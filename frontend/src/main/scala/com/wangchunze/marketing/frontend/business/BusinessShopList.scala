package com.neo.sk.feeler3.frontend.business

import com.neo.sk.feeler3.frontend.business.panel.{MyShopPanel, ShareShopPanel}
import com.neo.sk.feeler3.frontend.utils.{Http, JsFunc, Modal}
import com.neo.sk.feeler3.frontend.{Component, Route}
import io.circe.Json
import org.scalajs.dom.MouseEvent
import org.scalajs.dom.html._

import scala.collection.mutable

/**
  * Created by hongruying on 2017/3/24.
  */
object BusinessShopList extends Component[Div]{
  import scalatags.JsDom.short._
  import io.circe.generic.auto._
  import io.circe.syntax._

  import concurrent.ExecutionContext.Implicits.global
  import com.neo.sk.feeler3.ptcl.ShopProtocol._
  import com.neo.sk.feeler3.frontend.utils.MyUtil._

  private val modalContainer = div().render

  private val enterShopFun: (Long)=>Unit = { shopId =>
    val shopDetailPage = new BusinessShopDetail(shopId)

    this.hide
    shopDetailContainer.innerHTML = ""
    shopDetailContainer.appendChild(shopDetailPage.render())
    shopDetailPage.show
  }

  private val myShopComponent = new MyShopPanel(enterShopFun)

  private val shareShopComponent = new ShareShopPanel
  //我的店铺组件
//  object myShopComponent extends Component[Div]{
//    import ShopModel._
//    val shopMap = new mutable.HashMap[Long,(ShopInfo,Element)]()
//    val pageSize = 5
//    val tableBar = div(*.cls:="panel-heading shop-panel-heading")(
//      span(*.cls:="panel-heading__title")(
//        "我的店铺"
//      ),
//      span(*.cls:="panel-heading__button")(
//        button(*.cls:="btn btn-primary",*.onclick:=showCreateShopModal)("创建")
//      )
//    ).render
//
//    private[this] def showCreateShopModal = { e: MouseEvent =>
//      e.preventDefault()
//      val modalInputBox = input(
//        *.`type` := "text",
//        *.cls := "form-control",
//        *.placeholder:="店铺名称"
//      ).render
//
//      val successFunc: () => Unit = { () =>
//        val newShopName = modalInputBox.value
//        val data = Json.obj(
//          ("shopId",0.asJson),
//          ("shopName",newShopName.asJson),
//          ("boxName","".asJson),
//          ("boxMac","".asJson)
//        ).noSpaces
//
//        val url = Route.baseUrl + s"/shop/createShop"
//
//        Http.postJsonAndParse[CreateShopRsp](url,data).map{
//          case Right(rsp) =>
//            if(rsp.errCode == 0){//重新刷新店铺列表
//              val d = shopMap.map(_._2._1).toSeq :+ rsp.data.get
//              makeShopTbody(d)
//            }else{
//              println(s"create shop  error: ${rsp.msg}")
//              JsFunc.alert(s"create shop  error: msg=${rsp.msg}")
//            }
//          case Left(error) =>
//            println(s"create shop  error:$error")
//            JsFunc.alert(s"create shop  error:$error")
//        }
//      }
//
//      val modal = new Modal(
//        div(h2("创建店铺")).render,
//        div(*.cls := "form-group")(
//          label(*.`for` := "0")("新店铺名称").render,
//          modalInputBox
//        ).render,
//        successFunc,
//        "0"
//      )
//
//      modalContainer.innerHTML = ""
//      modalContainer.appendChild(modal.render())
//      modal.show
//    }
//
//    val shopTbody = tbody().render
//    val paginationContainer = div(*.textAlign:="center").render
//    val shopTable = div(*.cls:="panel-body")(
//      table(*.cls:="table table-striped")(
//        colgroup(*.colspan:="3")(
//          col(*.cls:="myshop-table-name"),
//          col(*.cls:="myshop-table-time"),
//          col(*.cls:="myshop-table-operator")
//        ),
//        thead()(
//          tr()(
//            th("店铺名称"),
//            th("创建时间"),
//            th("操作")
//          )
//        ),
//        shopTbody
//      ),
//      paginationContainer
//    ).render
//
//
//    //插入一个店铺
//    private [this] def insertNewShop(shop:ShopInfo) ={
//      val newRow = tr(
//        td()(
//          shop.shopName
//        ),
//        td()(
//          dataFormatDefault(shop.createTime)
//        ),
//        td()(
//          button(*.cls:="btn btn-link",*.onclick:=enterShopDetail(shop.id))("管理").render
//        )
//      ).render
//      shopMap.put(shop.id,(shop,newRow))
//      shopTbody.appendChild(newRow)
//    }
//
//    //更新店铺名称
//    def updateShopName(id:Long,shopName:String) = {
//      val shopOpt = shopMap.get(id)
//      if(shopOpt.isDefined){
//        val value = shopOpt.get
//        value._2.innerHTML = ""
//        value._2.appendChild(
//          td()(
//            shopName
//          ).render
//        )
//        value._2.appendChild(
//          td()(
//            dataFormatDefault(value._1.createTime)
//          ).render
//        )
//        value._2.appendChild(
//          td()(
//            button(*.cls:="btn btn-link",*.onclick:=enterShopDetail(id))("管理").render
//          ).render
//        )
//        shopMap.put(id,(value._1.copy(shopName = shopName),value._2))
//      }else{
//        println(s"update Shop Name error:id=$id is not exists")
//      }
//    }
//
//    //将数据渲染
//    def makeShopTbody(shops:Seq[ShopInfo]) = {
//      shopTbody.innerHTML = ""
//      shops.sortBy(_.createTime).map{ shop =>
//        val newRow = tr(
//          td()(
//            shop.shopName
//          ),
//          td()(
//            dataFormatDefault(shop.createTime)
//          ),
//          td()(
//            button(*.cls:="btn btn-link",*.onclick:=enterShopDetail(shop.id))("管理").render
//          )
//        ).render
//        shopMap.put(shop.id,(shop,newRow))
////        shopTbody.firstChild.appendChild(newRow)
//      }
//
//      if(shopMap.isEmpty){
//        val noData = div(*.cls:="shop-no-data")(
//          "暂无数据"
//        ).render
//        shopTable.firstChild.appendChild(noData)
//      }
//
//      paginationContainer.innerHTML = ""
//      val pagination = new Pagination(shopMap.size,pageSize,showPageContent)
//      paginationContainer.appendChild(pagination.render())
//    }
//
//    val showPageContent :Int =>Unit ={ x =>
//      shopTbody.innerHTML = ""
//      val content = shopMap.toList.drop((x-1)*pageSize).take(pageSize)
//      content.map{c =>
//        shopTbody.appendChild(c._2._2)
//      }
//    }
//
//    def enterShopDetail(shopId:Long) = { e:MouseEvent =>
//      e.preventDefault()
//      val shopDetailPage = new BusinessShopDetail(shopId)
//      hide
//      shopDetailContainer.innerHTML = ""
//      shopDetailContainer.appendChild(shopDetailPage.render())
//      shopDetailPage.show
//    }
//
//    override def render() = {
//      div(*.cls:="panel panel-default")(
//        tableBar,
//        shopTable
//      ).render
//    }
//  }

  //共享店铺组件
//  object shareShopComponent extends Component[Div]{
//    import ShopModel._
//    val shopMap = new mutable.HashMap[Long,(ShopInfo,Element)]()
//    val pageSize = 5
//    val tableBar = div(*.cls:="panel-heading shop-panel-heading")(
//      span(*.cls:="panel-heading__title")(
//        "共享店铺"
//      ),
//      span(*.cls:="panel-heading__button")(
//        button(*.cls:="btn btn-primary",*.onclick:=showAddShareShopModal)("添加共享店铺")
//      )
//    )
//
//    private[this] def showAddShareShopModal = { e: MouseEvent =>
//      e.preventDefault()
//      val modalInputBox = input(
//        *.`type` := "text",
//        *.cls := "form-control",
//        *.placeholder:="分享码"
//      ).render
//
//      val successFunc: () => Unit = { () =>
//        val shareCode = modalInputBox.value
//        val url = Route.baseUrl + s"/shop/checkShareCode?shareCode=$shareCode"
//
//        Http.getAndParse[CheckShareCodeRsp](url).map{
//          case Right(rsp) =>
//            if(rsp.errCode == 0){//重新刷新店铺列表
//              val d = shopMap.map(_._2._1).toSeq :+ rsp.data.get
//              makeShopTbody(d)
//            }else{
//              println(s"添加共享店铺错误: ${rsp.msg}")
//              JsFunc.alert(s"添加共享店铺错误: msg=${rsp.msg}")
//            }
//          case Left(error) =>
//            println(s"添加共享店铺错误:$error")
//            JsFunc.alert(s"添加共享店铺错误:$error")
//        }
//      }
//
//      val modal = new Modal(
//        div(h2("添加共享店铺")).render,
//        div(*.cls := "form-group")(
//          label(*.`for` := "0")("请输入店铺分享码：").render,
//          modalInputBox
//        ).render,
//        successFunc,
//        "0"
//      )
//
//      modalContainer.innerHTML = ""
//      modalContainer.appendChild(modal.render())
//      modal.show
//    }
//
//    val shopTbody = tbody().render
//    val paginationContainer = div(*.textAlign:="center").render
//    val shopTable = div(*.cls:="panel-body")(
//      table(*.cls:="table table-striped")(
//        colgroup(*.colspan:="2")(
//          col(*.cls:="shareshop-table-name"),
//          col(*.cls:="shareshop-table-time")
//        ),
//        thead()(
//          tr()(
//            th("店铺名称"),
//            th("创建时间")
//          )
//        ),
//        shopTbody
//      ),
//      paginationContainer
//    ).render
//
//    //插入一个店铺
//    private [this] def insertNewShop(shop:ShopInfo) ={
//      val newRow = tr(
//        td()(
//          shop.shopName
//        ),
//        td()(
//          dataFormatDefault(shop.createTime)
//        )
//      ).render
//      shopMap.put(shop.id,(shop,newRow))
//      shopTbody.appendChild(newRow)
//    }
//
//    //更新店铺名称
//    def updateShopName(id:Long,shopName:String) = {
//      val shopOpt = shopMap.get(id)
//      if(shopOpt.isDefined){
//        val value = shopOpt.get
//        value._2.innerHTML = ""
//        value._2.appendChild(
//          td()(
//            shopName
//          ).render
//        )
//        value._2.appendChild(
//          td()(
//            dataFormatDefault(value._1.createTime)
//          ).render
//        )
//        shopMap.put(id,(value._1.copy(shopName = shopName),value._2))
//      }else{
//        println(s"update Shop Name error:id=$id is not exists")
//      }
//    }
//
//    //将数据渲染
//    def makeShopTbody(shops:Seq[ShopInfo]) = {
//      shopTbody.innerHTML = ""
//      shops.sortBy(_.createTime).map{ shop =>
//        val newRow = tr(
//          td()(
//            shop.shopName
//          ),
//          td()(
//            dataFormatDefault(shop.createTime)
//          )
//        ).render
//        shopMap.put(shop.id,(shop,newRow))
////        shopTbody.appendChild(newRow)
//      }
//
//      if(shopMap.isEmpty){
//        val noData = div(*.cls:="shop-no-data")(
//          "暂无数据"
//        ).render
//        shopTable.firstChild.appendChild(noData)
//      }
//
//      paginationContainer.innerHTML = ""
//      val pagination = new Pagination(shopMap.size,pageSize,showPageContent)
//      paginationContainer.appendChild(pagination.render())
//
//    }
//
//    val showPageContent :Int =>Unit ={ x =>
//      shopTbody.innerHTML = ""
//      val content = shopMap.toList.drop((x-1)*pageSize).take(pageSize)
//      content.map{c =>
//        shopTbody.appendChild(c._2._2)
//      }
//    }
//
//    override def render() = {
//      div(*.cls:="panel panel-default")(
//        tableBar,
//        shopTable
//      ).render
//    }
//  }

  def getShopList = {
    val url = Route.baseUrl + s"/shop/shopList"
//    val url = s"http://10.1.29.250:30173/feeler3/shop/shopList"
    Http.getAndParse[GetShopListRsp](url).map{
      case Right(rsp) =>
        if(rsp.errCode == 0){
          myShopComponent.makeShopTbody(rsp.data.get.merchantShop)
          shareShopComponent.makeShopTbody(rsp.data.get.observedShop)
        }else{
          println(s"get shop list error: ${rsp.msg}")
          JsFunc.alert(s"get shop list error: msg=${rsp.msg}")
        }
      case Left(error) =>
        println(s"get shop list error:$error")
        JsFunc.alert(s"get shop list error:$error")
    }
  }

  val breadcrumb = ol(*.cls:="breadcrumb none-margin")(
    li(*.cls:="active")(
      "店铺列表"
    )
  )

  val page = div(*.id:="shop-list")(
    breadcrumb,
    myShopComponent.render(),
    shareShopComponent.render()
  ).render

  def show : Unit = {
    getShopList
    page.setAttribute("style","display:block")
    shopDetailContainer.innerHTML = ""
  }

  def hide : Unit = {
    page.setAttribute("style","display:none")
  }

  val shopDetailContainer = div().render

  val container = div(*.cls:="shop-container none-padding none-margin")(
    div(*.cls:="row")(
      div(*.cls:="col-md-12")(
        page,
        shopDetailContainer,
        modalContainer
      )
    )
  ).render



  override def render() = {
    show
    container
  }

}
