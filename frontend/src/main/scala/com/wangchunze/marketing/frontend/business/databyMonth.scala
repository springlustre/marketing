...
object databyMonth extends Component[Div] {
...
/**
  ...
  * */
  ...
  /**
    * option
    * */
  val datePick = input(*.id := "Date",*.cls:="Wdate",*.height:="auto", *.`type` := "text",*.onclick:="WdatePicker({ dateFmt: 'yyyy-MM'})",*.readonly:="readonly").render
  val ensure = button(*.onclick:=pick,*.border:="none",*.background:="#fff",*.color:="#3782ff",*.marginRight:="20px")("确认").render
  val yesterday = button(*.cls:="btn btn-primary",*.id:="fore",*.onclick:=yest)("前一月").render
  val tomorrow = button(*.cls:="btn btn-primary",*.id:="next",*.onclick:=tomo)("后一月").render
  ...
  def yest={event:MouseEvent =>
    ...
    var yes = MyUtil.DateFormatter(new Date(timestamp-86400000*30),"YYYY-MM")
    val date = datePick.value
    val year = date.split("-").head
    val month = date.split("-")(1)
    if(month.toInt>1){
      val foreMonth = month.toInt - 1
      if(foreMonth<10)
        yes = year + "-0" + foreMonth.toString
      else
        yes = year + "-" + foreMonth.toString
    }
    else {
      val foreYear = year.toInt - 1
      yes = foreYear.toString + "-12"
    }
    datePick.value = yes
    val first = yes.split("-").head + "-" + yes.split("-")(1) + "-" + "01"
    val last = yes.split("-").head + "-" + yes.split("-")(1) + "-" + new Date(yes.split("-").head.toInt,yes.split("-")(1).toInt,0).getDate()
    updateFlowDetail(id.toString,first,last)
    updateAverageTime(id.toString,first,last)
    updateResidentTime(id.toString,yes)
    updateRatio(id.toString,first,last)
    updateGuestConversion(id.toString,first,last)
    updateFreq(id,yes)
  }
  def tomo={event:MouseEvent =>
    ...
    val st = MyUtil.dataFormatDefault(nowdate,"YYYY-MM")
    println(s"data=${datePick.value},st=$st")
    if(datePick.value>=st){
      JsFunc.alert("不可超过当前日期")
    }
    else{
      var to = datePick.value
      val date = datePick.value
      val year = date.split("-").head
      val month = date.split("-")(1)
      if(month.toInt<12){
        val nextMonth = month.toInt + 1
        if(nextMonth<10)
          to = year + "-0" + nextMonth.toString
        else
          to = year + "-" + nextMonth.toString
      }
      else {
        val nextYear = year.toInt + 1
        to = nextYear.toString + "-01"
      }
      datePick.value = to
      val first = to.split("-").head + "-" + to.split("-")(1) + "-" + "01"
      val last = to.split("-").head + "-" + to.split("-")(1) + "-" + new Date(to.split("-").head.toInt,to.split("-")(1).toInt,0).getDate()
      updateFlowDetail(id.toString,first,last)
      updateAverageTime(id.toString,first,last)
      updateResidentTime(id.toString,to)
      updateRatio(id.toString,first,last)
      updateGuestConversion(id.toString,first,last)
      updateFreq(id,to)
    }
  ...
  val freqChart = canvas(*.width := "500").render
  ...
  private var freqInstance: Option[Chart] = None
  //data
  ...
  private var freqData = List.empty[Freq]
  //draw
  ...
  private def drawFreqChart(area: Canvas,data:List[Freq]) = {

    import js.JSConverters._
    println("drawbarChart")
    val ctx = area.getContext("2d").asInstanceOf[CanvasRenderingContext2D]
    val (xs, ys) = data.sortBy(_.kind).map(d => (freqMap(d.kind), d.value.toInt)).unzip
    val dataSet = new BarDataSet(data = ys.toJSArray)
    val chartData = new ChartData(xs.toJSArray, js.Array(dataSet))
    freqInstance.foreach(_.destroy())
    freqInstance = Some(new Chart(ctx, new ChartConfig("horizontalBar", chartData,null)))
    println("drawbarChart error")
  }
  ...
  private def updateFreq(id: String,date:String) = {
    val url = UserRoutes.freq(id,date)
    Http.getAndParse[FreqRsp](url).foreach {
      case Right(rsp) =>
        if (rsp.errCode == 0) {
          freqData = rsp.data
          drawFreqChart(freqChart,freqData)
        } else {
          JsFunc.alert(s"some error: ${rsp.msg}")
        }
      case Left(err) =>
  ...
  private def todayDate = MyUtil.DateFormatter(new Date(),"YYYY-MM")
  ...
  override def render() = {
    ...
    getShopList
    val year = todayDate.split("-").head
    val month = todayDate.split("-")(1)
    val first = year + "-" + month + "-" + "01"
    val last = year + "-" + month + "-" + new Date(year.toInt,month.toInt,0).getDate()
    ...
    updateResidentTime(window.localStorage.getItem("shopId").toString,todayDate)
    ...
    updateFreq(window.localStorage.getItem("shopId").toString,todayDate)
    //updateFreq(window.localStorage.getItem("shopId").toString,)
